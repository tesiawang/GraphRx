...
parser = argparse.ArgumentParser(description='Generate wireless communication data for offline pretraining or online adaptation')

# Data generation mode control
parser.add_argument('--mode', type=str, default='online', choices=['offline', 'online'], 
                    help='Data generation mode: "offline" generates data for each SNR point separately, "online" generates data across SNR ranges for channel adaptation')

# Signal-to-Interference Ratio (SIR) configuration
parser.add_argument('--generation_type', type=int, default=0, choices=[0, 1, 2],
                    help='Generator interference type: 0=NoneSIR (no interference), 1=MidSIR (medium interference), 2=HighSIR (low interference)')

# Random seed for reproducibility
parser.add_argument('--seed', type=int, default=42, 
                    help='Random seed for reproducible data generation across numpy and tensorflow')

# Batch size configuration
parser.add_argument('--tti_batch_size', type=int, default=32, 
                    help='Transmission Time Interval (TTI) batch size - number of samples per batch (each sample contains pilot, received signal, noise, and codeword bits)')

# Data quantity control for different modes
parser.add_argument('--num_batches_per_config', type=int, required=False, 
                    help='Number of batches to generate per channel configuration (REQUIRED for online mode). On HPC: typically 400')

parser.add_argument('--num_batches_per_snr', type=int, required=False, 
                    help='Number of batches to generate per SNR point (REQUIRED for offline mode). On HPC: typically 200')
run_args = parser.parse_args()


# Enforce mode-specific parameter requirements
if run_args.mode == 'offline':
    if run_args.num_batches_per_snr is None:
        parser.error("--num_batches_per_snr is required when --mode=offline")
    if run_args.num_batches_per_config is not None:
        print(f"WARNING: --num_batches_per_config={run_args.num_batches_per_config} is ignored in offline mode")
    print(f"✓ Offline mode: Using --num_batches_per_snr={run_args.num_batches_per_snr}")

elif run_args.mode == 'online':
    if run_args.num_batches_per_config is None:
        parser.error("--num_batches_per_config is required when --mode=online")
    if run_args.num_batches_per_snr is not None:
        print(f"WARNING: --num_batches_per_snr={run_args.num_batches_per_snr} is ignored in online mode")
    print(f"✓ Online mode: Using --num_batches_per_config={run_args.num_batches_per_config}")
...
if __name__ == "__main__":
    ...
    # Below is setting up the DataGenerator with a full snr range 
    ...
    # ---------------------------- Generate online data (change to online snr range)---------------------------- #
    elif run_args.mode == 'online':
        ...
        raise ValueError(f"Invalid mode '{run_args.mode}'. Please choose from 'offline' or 'online'.")
