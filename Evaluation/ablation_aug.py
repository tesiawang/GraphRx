...
parser.add_argument("--aug_times", type=int, default=4, required=False)
...
if __name__ == "__main__":
    ...
    run = wandb.init(project='Infocom2025', config=wandb_config, 
                        name="AblationEval_Dist{:d}_Aug{:d}".format(run_args.client_data_dist_type,run_args.aug_times))
    ...
    eval_result_save_folder = root_path + "/Evaluation/Ablation_Aug/Dist{:d}".format(run_args.client_data_dist_type)
    ...
    if run_args.dist_metric == "l2":
        net_path = root_path + "/OnlineAdapt/FedGraphs/AugTest_HighSIR/new_AdaptedModels_Dist{:d}_l2_c0.20_Aug{:d}_CS0".format(run_args.client_data_dist_type, run_args.aug_times)
    else:
    ...
    for client_id in range(run_args.num_total_clients):
        ...
        if run_args.dist_metric == "l2":
            result_save_path = [eval_result_save_folder + "/" + "aug{:d}_bs{:d}.pkl".format(run_args.aug_times,client_id)]
        else: 
            raise ValueError("Invalid distance metric")
        ...
        print("FedGraph_L2 with aug_times = {:d}, Client {:d}, BER {}".format(run_args.aug_times, client_id, ber))
